...
/**
 * astro-icon：避免对整包使用 ["*"]（会加载巨量 Iconify JSON，明显拖慢 `npm run dev`）。
 * 在 Astro 里使用新的 `<Icon name="集合:名称" />` 或配置里的动态图标时，把「名称」加到对应集合数组。
 */
const astroIconInclude = {
	...
	"fa7-brands": ["alipay", "creative-commons", "gitee", "github", "qq", "weixin"],
...
export default defineConfig({
	...
	vite: {
		...
		server: {
			watch: {
				ignored: ["**/package/**", "**/Firefly-docs/**", "**/dist/**", "**/.astro/**"],
			},
		},
		resolve: {
			alias: {
